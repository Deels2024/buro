...
export function OperationalDrawer({id,kind,onClose,onChanged}:{id:string;kind:"listing"|"claim"|"ticket";onClose:()=>void;onChanged:()=>void}) {
  ...
  useEffect(()=>{
    ...
    async function load(){
      try{
        ...
        if(kind==="ticket"){const [r,ticket]=await Promise.all([api<Message[]>(`/support/tickets/${id}/messages`),api<{guest_contact:string|null}>(`/admin/support/tickets/${id}`)]);if(active){setMessages(r);setGuestContact(ticket.guest_contact);}}
  ...
  async function send(e:FormEvent){e.preventDefault();await run(async()=>{await api(`/support/tickets/${id}/messages`,"POST",{body:text,internal:guestContact!==null,attachment_ids:[]});setText("");setMessages(await api<Message[]>(`/support/tickets/${id}/messages`));setNotice(guestContact ? "Внутренняя заметка сохранена" : "Ответ отправлен");});}
  return <dialog ref={dialog} className="operational-dialog" onCancel={onClose} aria-label={kind==="claim"?"Проверка владельца":kind==="listing"?"Публикация":"Переписка поддержки"}>
    ...
    {kind==="ticket"&&!loading&&<section className="operation-form">{guestContact&&<p><strong>Обращение без входа.</strong> Свяжитесь с заявителем по контакту: <span>{guestContact}</span>. Сообщения ниже сохраняются как внутренние заметки.</p>}<div className="support-messages">{messages.map(m=><article key={m.id}><small>{new Date(m.created_at).toLocaleString("ru-RU")}{m.internal?" · Внутренняя заметка":""}</small><p>{m.body}</p></article>)}</div><form onSubmit={send}><label>{guestContact ? "Внутренняя заметка" : "Ответ пользователю"}<textarea value={text} onChange={e=>setText(e.target.value)} minLength={1} maxLength={4000} required/></label><div className="modal-actions"><button className="primary-button" disabled={busy}>{guestContact ? "Сохранить заметку" : "Отправить ответ"}</button><button type="button" className="filter-button" disabled={busy} onClick={()=>void run(async()=>{await api(`/admin/support/tickets/${id}`,"PATCH",{status:"resolved"});setNotice("Обращение решено");})}>Отметить решённым</button></div></form></section>}
  </dialog>;
}
