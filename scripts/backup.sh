...
if [ -f .releases/current ]; then cp .releases/current "$staging/active-sha"; fi
docker compose unpause minio api worker >/dev/null
paused=false
archive="$backup_dir/bureau-$(date -u +%Y%m%dT%H%M%SZ).tar.age"
tar -C "$staging" -cf "$staging/payload.tar" database.dump media.tar secrets.tar .env docker-compose.yml nginx.conf checkout-sha
